...
function core(){
     ...
     *callback：回调函数
     */
    function topTips(content, duration, callback) {
        weui.topTips(content || '错误信息', {
     ...
     * ids，例如aac002,aac001
     * </br>
     * data，JSON数组，例如[{'label':'男','value':'1'},{'label':'女','value':'2'}]
     */
    function setSelectInputList(ids, data) {
        ...
        for (var i = 0; i < _ids.length; i++) {
            ...
            if (obj.attr("mtype") != "selectInput") {
                ...
                Base.selectDatas[cur_id+'_showPicker'] = data;
                $("#" + cur_id + "_showPicker").off("click");
                $("#" + cur_id + "_showPicker").click(function() {
                    ...
                    weui.picker(data, {
                        ...
                        defaultValue : eval('(' + _input.attr("defaultvalue") + ')'),
                        onConfirm : function(result) {
                            var onselect = eval(_input.attr("onselect"));
                            if (typeof eval(onselect) === 'function') {
                                var ret = eval(onselect).call(this, result[result.length - 1].label, result[result.length - 1].value,obj);
                                if (ret == false) {
                                    _desc.val("");
                                    _input.val("");
                                    return false;
                            ...
                            //设置值后隐藏校验错误提示
                            weui.form.hideErrorTips(document.getElementById(cur_id + '_DESC'));
                        },
                        title : '选择' + _input.attr("key"),
                        id : cur_id + '_showPicker'
                    });
                });
            }
        }
            ...
            for (var i = 0; i < _ids.length; i++) {
                ...
                if (obj.attr("mtype") == "selectInput") {
                    ...
                    if(Base.isEmpty(value)){
                        ...
                        let codeList = Base.selectDatas[cur_id + '_showPicker'];
                        if (codeList) {
                            var findCode = false;
    ...
    function alert(message, type, callback) {
        ...
        window.weui_alert = weui.alert(message, function() {
            window.weui_alert.hide(function() {
                if (typeof callback === "function") {
                    callback.call(this, []);
                }
            });
    ...
    function showMask(message) {
        return weui.loading(message || '读取中...');
    }

    /**
     * 取消遮罩层
     */
    function hideMask(loading, callback) {
        if (loading) {
    ...
    function submit(id, url, parameter, onsubmit, autoValidate, success, error, showMask, async) {
        ...
        if(Base.isEmpty(id)){
            ...
            if (autoValidate) {
                Base.validateForm(id, function(yes) {
                    if (yes) {
                        var data;
                        if (id) {
                            data = $("#" + id).serialize();
